...
public class GenericElementNode<T> implements GenericListADTNode<T> {
  ...
  @Override
  public GenericListADTNode<T> reverse(GenericListADTNode<T> tail) {
    GenericListADTNode tmpHead = tail.addFront(this.object);
    return this.rest.reverse(tmpHead);
  }

  @Override
  public GenericListADTNode<T> swap(int leftIndex, int rightIndex) {
    T tmp1 = this.get(leftIndex);
    T tmp2 = this.get(rightIndex);
    this.remove(this.get(leftIndex));
    this.remove(this.get(rightIndex));

    this.add(rightIndex, tmp1);
    this.add(leftIndex, tmp2);
